...
async def upload_files(files: List[UploadFile] = File(...), disk_config: str = Form(None)):
    ...
    try:
        ...
        import tempfile
        tmpdir = os.path.join(tempfile.gettempdir(), "zirsave_uploads")
        ...
        for up in files:
            ...
            # parse (guard against malformed STL)
            try:
                parsed = parse_stl(path, disk_thickness=config.get("thickness", 20.0))
            except Exception as e:
                # record as unparsed/unfeasible and continue
                file_id = str(uuid.uuid4())
                file_rec = {"id": file_id, "filename": up.filename, "path": path, "bbox_w": None, "bbox_h": None, "depth": None, "feasible": False, "error": str(e)}
                FILES[file_id] = file_rec
                try:
                    with db.SessionLocal() as session:
                        session.execute(models.files.insert().values(id=file_id, case_id=case_id, filename=up.filename, path=path, bbox_w=None, bbox_h=None, depth=None, feasible=0))
                        session.commit()
                except Exception:
                    if case_id in CASES:
